...
-- ------------------------------
-- SEANCE_PLACES (CONFIGURATION PAR SÉANCE)
-- ------------------------------
CREATE TABLE seance_place (
  id_seance INT NOT NULL REFERENCES seance(id) ON DELETE CASCADE,
  id_place INT NOT NULL REFERENCES place(id) ON DELETE CASCADE,
  id_type_place INT NOT NULL REFERENCES type_place(id),
  PRIMARY KEY (id_seance, id_place)
);

CREATE INDEX idx_seance_place_seance ON seance_place(id_seance);
CREATE INDEX idx_seance_place_place ON seance_place(id_place);
...
BEGIN
  ...
  FOR num IN 1..100 LOOP
    IF num BETWEEN 1 AND 10 THEN
      type_id := type_vip_id;
    ELSIF num BETWEEN 11 AND 30 THEN
      type_id := type_pmr_id;
    ELSE
      type_id := type_standard_id;
    END IF;
